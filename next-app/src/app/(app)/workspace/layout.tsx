      ...
      >
        <nav className="px-2.5 pt-1.5 backdrop-blur-xl">
          <div className="flex items-center gap-1">
            {mobile && (
              <div className={`${showSidebar ? 'md:hidden' : ''} self-center flex flex-none items-center`}>
            ...
            <div>
              <div className="flex gap-1 scrollbar-none overflow-x-auto w-fit text-center text-sm font-medium rounded-full bg-transparent py-1 touch-auto pointer-events-auto">
                ...
                {canViewKnowledge && (
                  <Link className={getLinkClass('/workspace/knowledge')} href="/workspace/knowledge">
                    Knowledge
                  </Link>
                )}
                {canViewPrompts && (
        ...
        <div className="pb-1 px-3 md:px-[18px] flex-1 max-h-full overflow-y-auto" id="workspace-container">
          {children}
        </div>
      </div>
    </>
