<script lang="ts">
	...
	const submitHandler = async () => {
		...
		if (
			...
			toast.error('Document Intelligence endpoint required.');
			return;
		}
		if (
		...
		if (
			...
			toast.error('MinerU API Key required for Cloud API mode.');
			return;
		}
	...
	{#if RAGConfig}
		<div class=" space-y-2.5 overflow-y-scroll scrollbar-hidden h-full pr-1.5">
			<div class="">
					...
					<div class="mb-2.5 flex flex-col w-full justify-between">
						...
						{:else if RAGConfig.CONTENT_EXTRACTION_ENGINE === 'datalab_marker'}
							<div class="my-0.5 flex gap-2 pr-2">
								<Tooltip
									content={'API Base URL for Datalab Marker service. Defaults to: https://www.datalab.to/api/v1/marker'}
									placement="top-start"
									className="w-full"
								>
									<input
							...
							<div class="flex justify-between w-full mt-2">
								<div class="self-center text-xs font-medium">
									<Tooltip
										content={'Significantly improves accuracy by using an LLM to enhance tables, forms, inline math, and layout detection. Will increase latency. Defaults to False.'}
										placement="top-start"
									>
										{'Use LLM'}
									</Tooltip>
							...
							<div class="flex justify-between w-full mt-2">
								<div class="self-center text-xs font-medium">
									<Tooltip
										content={'Skip the cache and re-run the inference. Defaults to False.'}
										placement="top-start"
									>
										{'Skip Cache'}
									</Tooltip>
							...
							<div class="flex justify-between w-full mt-2">
								<div class="self-center text-xs font-medium">
									<Tooltip
										content={'Force OCR on all pages of the PDF. This can lead to worse results if you have good text in your PDFs. Defaults to False.'}
										placement="top-start"
									>
										{'Force OCR'}
									</Tooltip>
							...
							<div class="flex justify-between w-full mt-2">
								<div class="self-center text-xs font-medium">
									<Tooltip
										content={'Whether to paginate the output. Each page will be separated by a horizontal rule and page number. Defaults to False.'}
										placement="top-start"
									>
										{'Paginate'}
									</Tooltip>
							...
							<div class="flex justify-between w-full mt-2">
								<div class="self-center text-xs font-medium">
									<Tooltip
										content={'Strip existing OCR text from the PDF and re-run OCR. Ignored if Force OCR is enabled. Defaults to False.'}
										placement="top-start"
									>
										{'Strip Existing OCR'}
									</Tooltip>
							...
							<div class="flex justify-between w-full mt-2">
								<div class="self-center text-xs font-medium">
									<Tooltip
										content={'Disable image extraction from the PDF. If Use LLM is enabled, images will be automatically captioned. Defaults to False.'}
										placement="top-start"
									>
							...
							<div class="flex justify-between w-full mt-2">
								<div class="self-center text-xs font-medium">
									<Tooltip
										content={'Format the lines in the output. Defaults to False. If set to True, the lines will be formatted to detect inline math and styles.'}
										placement="top-start"
									>
										{'Format Lines'}
									</Tooltip>
							...
							<div class="flex justify-between w-full mt-2">
								<div class="self-center text-xs font-medium">
									<Tooltip
										content={"The output format for the text. Can be 'json', 'markdown', or 'html'. Defaults to 'markdown'."}
										placement="top-start"
									>
										{'Output Format'}
									</Tooltip>
						...
						{:else if RAGConfig.CONTENT_EXTRACTION_ENGINE === 'mineru'}
							...
							<div class="flex flex-col justify-between w-full mt-2">
								<div class="text-xs font-medium">
									<Tooltip
										content={'Advanced parameters for MinerU parsing (enable_ocr, enable_formula, enable_table, language, model_version, page_ranges)'}
										placement="top-start"
									>
										{'Parameters'}
									</Tooltip>
				...
				{#if !RAGConfig.BYPASS_EMBEDDING_AND_RETRIEVAL}
					<div class="mb-3">
						<div class=" mt-0.5 mb-2.5 text-base font-medium">{'Embedding'}</div>
								...
								<div class="flex items-center relative">
									<select
										...
										placeholder={'Select an embedding model engine'}
                                                                                on:change={(e) => {
                                                                                        if (['openai', 'azure_openai'].includes(e.target.value)) {
								...
								<div class="my-0.5 flex flex-col gap-2 pr-2 w-full">
									<div class="flex gap-2">
										<input
											...
											required
										/>
										<SensitiveInput placeholder={'API Key'} bind:value={AzureOpenAIKey} />
									</div>
									<div class="flex gap-2">
										<input
											class="flex-1 w-full text-sm bg-transparent outline-hidden"
						...
						<div class="  mb-2.5 flex flex-col w-full">
                                                                                ...
                                                                                >
                                                                                        ...
                                                                                        {:else}
                                                                                                ...
                                                                                                >
                                                                                                        ...
                                                                                                        />
                                                                                                        <path
                                                                                                                d="M3.59.775a.75.75 0 0 0-1.5 0v1.5A2.75 2.75 0 0 0 4.75 14h6.5A2.75 2.75 0 0 0 14 11.25v-1.5a.75.75 0 0 0-1.5 0v1.5c0 .69-.56 1.25-1.25 1.25h-6.5c-.69 0-1.25-.56-1.25-1.25v-1.5Z"
                                                                                                        />
                                                                                                </svg>
                                                                                        {/if}
                                                                                </button>
							...
							<div class="  mb-2.5 flex w-full justify-between">
								<div class="self-center text-xs font-medium">
									<Tooltip
										content={'Runs embedding tasks concurrently to speed up processing. Turn off if rate limits become an issue.'}
										placement="top-start"
									>
					...
					<div class="mb-3">
						<div class=" mt-0.5 mb-2.5 text-base font-medium">{'Retrieval'}</div>
						...
						{#if !RAGConfig.RAG_FULL_CONTEXT}
							...
							{#if RAGConfig.ENABLE_RAG_HYBRID_SEARCH === true}
								<div class="mb-2.5 flex w-full justify-between">
									...
									<div class="flex items-center relative">
										<Tooltip
											content={'Adds filenames, titles, sections, and snippets into the BM25 text to improve lexical recall.'}
										>
							...
							{#if RAGConfig.ENABLE_RAG_HYBRID_SEARCH === true}
								<div class=" mb-2.5 py-0.5 w-full justify-between">
									<Tooltip
										content={'The Weight of BM25 Hybrid Search. 0 more semantic, 1 more lexical. Default 0.5'}
										placement="top-start"
				...
				<div class="mb-3">
					<div class=" mt-0.5 mb-2.5 text-base font-medium">{'Files'}</div>

					<hr class=" border-gray-100 dark:border-gray-850 my-2" />

					<div class="  mb-2.5 flex w-full justify-between">
						<div class=" self-center text-xs font-medium">{'Allowed File Extensions'}</div>
						<div class="flex items-center relative">
							<Tooltip
								content={'Allowed file extensions for upload. Separate multiple extensions with commas. Leave empty for all file types.'}
								placement="top-start"
							>
								<input
									class="flex-1 w-full text-sm bg-transparent outline-hidden"
					...
					<div class="  mb-2.5 flex w-full justify-between">
						...
						<div class="flex items-center relative">
							<Tooltip
								content={'The maximum file size in MB. If the file size exceeds this limit, the file will not be uploaded.'}
								placement="top-start"
							>
								<input
									class="flex-1 w-full text-sm bg-transparent outline-hidden"
					...
					<div class="  mb-2.5 flex w-full justify-between">
						...
						<div class="flex items-center relative">
							<Tooltip
								content={'The maximum number of files that can be used at once in chat. If the number of files exceeds this limit, the files will not be uploaded.'}
								placement="top-start"
							>
								<input
									class="flex-1 w-full text-sm bg-transparent outline-hidden"
					...
					<div class="  mb-2.5 flex w-full justify-between">
						<div class=" self-center text-xs font-medium">{'Image Compression Width'}</div>
						<div class="flex items-center relative">
							<Tooltip
								content={'The width in pixels to compress images to. Leave empty for no compression.'}
								placement="top-start"
							>
								<input
									class="flex-1 w-full text-sm bg-transparent outline-hidden"
					...
					<div class="  mb-2.5 flex w-full justify-between">
						...
						<div class="flex items-center relative">
							<Tooltip
								content={'The height in pixels to compress images to. Leave empty for no compression.'}
								placement="top-start"
							>
								<input
									class="flex-1 w-full text-sm bg-transparent outline-hidden"
				...
				<div class="mb-3">
					<div class=" mt-0.5 mb-2.5 text-base font-medium">{'Integration'}</div>
				...
				<div class="mb-3">
					<div class=" mt-0.5 mb-2.5 text-base font-medium">{'Danger Zone'}</div>
