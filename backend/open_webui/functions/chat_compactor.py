...
class Valves(BaseModel):
    ...
    priority: int = Field(default=0, description="Priority level for the action")
...
class Action:
    ...
    async def action(
        ...
        if not messages:
            ...
            if __event_emitter__:
                await __event_emitter__(
                    {
                        "type": "notification",
                        "data": {"type": "error", "content": "No messages to compact"},
                    }
                )
        ...
        log.info(
            f"Created compacted message (id={compacted_message_id}, replaced {len(original_message_ids)} messages)"
        )
