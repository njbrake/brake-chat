...
async def get_note_list(request: Request, page: int | None = None, user=Depends(get_verified_user)):
    if user.role != "admin" and not has_permission(
        user.id, "features.notes", request.app.state.config.USER_PERMISSIONS
    ):
        raise HTTPException(
    ...
    notes = [
        ...
        for note in Notes.get_notes_by_permission(user.id, "write", skip=skip, limit=limit)
    ]
...
async def get_note_by_id(request: Request, id: str, user=Depends(get_verified_user)):
    ...
    if user.role != "admin" and (
        user.id != note.user_id and (not has_access(user.id, type="read", access_control=note.access_control))
    ):
...
async def update_note_by_id(request: Request, id: str, form_data: NoteForm, user=Depends(get_verified_user)):
    ...
    if user.role != "admin" and (
        user.id != note.user_id and not has_access(user.id, type="write", access_control=note.access_control)
    ):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=ERROR_MESSAGES.DEFAULT())
