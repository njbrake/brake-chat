...
async def query_doc_with_hybrid_search(
    ...
    try:
        ...
        distances = [d.metadata.get("score") for d in compressed_docs]
        documents = [d.page_content for d in compressed_docs]
        metadatas = [d.metadata for d in compressed_docs]
        ...
        if k < k_reranker:
            ...
            if sorted_items:
                distances_tuple, metadatas_tuple, documents_tuple = zip(*sorted_items)
                distances, metadatas, documents = (
                    list(distances_tuple),
                    list(metadatas_tuple),
                    list(documents_tuple),
                )
            else:
        ...
        log.info(f"query_doc_with_hybrid_search:result {result['metadatas']} {result['distances']}")
        return result
    except Exception as e:
...
async def get_sources_from_items(
    ...
    for item in items:
        ...
        if item.get("type") == "text":
            ...
            url_content, url_docs = get_content_from_url(request, item.get("url"))
            if url_docs:
                query_result = {
                    "documents": [[url_content]],
