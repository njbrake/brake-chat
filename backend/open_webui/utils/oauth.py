...
async def get_oauth_client_info_with_dynamic_client_registration(
    ...
    try:
        ...
        oauth_client_metadata = OAuthClientMetadata(
            client_name="BrakeChat",
            redirect_uris=[f"{redirect_base_url}/oauth/clients/{client_id}/callback"],  # type: ignore[list-item]
...
class OAuthManager:
    ...
    def update_user_groups(
        self, user: UserModel, user_data: dict[str, Any], default_permissions: dict[str, Any]
    ) -> None:
    ...
    async def handle_callback(self, request: Request, provider: str, response: Response) -> RedirectResponse:
        ...
        try:
            ...
            for existing_session in sessions:
                if existing_session.provider == provider:
                    OAuthSessions.delete_session_by_id(existing_session.id)
